...
{
    public sealed class UnityReadReportTool : IUnityMcpTool
    {
        ...
        public UnityMcpSchemaDeclaration InputSchema { get; } = new UnityMcpSchemaDeclaration
        {
            ...
            Value = UnityQueriesSchemas.ReadReport
        };

        public UnityMcpToolResult Execute(UnityMcpToolContext context, IUnityMcpCancellation cancellation)
        {
            cancellation?.ThrowIfCancellationRequested();
            if (!UnityQueriesJsonUtil.TryDeserializeArgs<UnityReadReportArgs>(context.RawArgsJson, out var args, out var failure))
            {
                return failure;
            }
            ...
            {
                return UnityQueriesResult.InvalidArgs("AGENTBRIDGE_REPORT_PATH_REQUIRED", "reportPath is required.");
            }

            if (args.offset < 0)
            {
                return UnityQueriesResult.InvalidArgs("AGENTBRIDGE_REPORT_OFFSET_INVALID", "offset must be greater than or equal to 0.");
            }

            if (args.limit <= 0 || args.limit > MaxLimit)
            {
                return UnityQueriesResult.InvalidArgs("AGENTBRIDGE_REPORT_LIMIT_INVALID", $"limit must be in the range 1..{MaxLimit}.");
            }

            if (args.maxBytes <= 0 || args.maxBytes > MaxMaxBytes)
            {
                return UnityQueriesResult.InvalidArgs("AGENTBRIDGE_REPORT_MAX_BYTES_INVALID", $"maxBytes must be in the range 1..{MaxMaxBytes}.");
            }

            if (!TryResolveReportPath(context, args.reportPath, out var absolutePath, out var normalizedReportPath, out failure))
            {
                return failure;
            }
            ...
            {
                return UnityQueriesResult.InvalidArgs("AGENTBRIDGE_REPORT_NOT_FOUND", "reportPath does not resolve to an existing Agent Bridge report.");
            }
            ...
            {
                return UnityQueriesResult.InvalidArgs("AGENTBRIDGE_REPORT_PARSE_FAILED", exception.Message);
            }

            if (!IsValidAgentBridgeReport(report))
            {
                return UnityQueriesResult.InvalidArgs("AGENTBRIDGE_REPORT_SHAPE_INVALID", "reportPath does not resolve to an Agent Bridge report payload.");
            }
            ...
            {
                return UnityQueriesResult.InvalidArgs("AGENTBRIDGE_REPORT_POINTER_INVALID", pointerFailure);
            }
        ...
        private static bool TryResolveReportPath(UnityMcpToolContext context, string requestedReportPath, out string absolutePath, out string normalizedReportPath, out UnityMcpToolResult failure)
        {
            absolutePath = null;
            ...
            {
                failure = UnityQueriesResult.InvalidArgs("AGENTBRIDGE_REPORT_PATH_ABSOLUTE", "Absolute report paths are not allowed.");
                return false;
            }

            if (!requestedReportPath.EndsWith(".json", StringComparison.OrdinalIgnoreCase))
            {
                failure = UnityQueriesResult.InvalidArgs("AGENTBRIDGE_REPORT_PATH_EXTENSION_INVALID", "reportPath must reference a .json file.");
                return false;
            }

            var projectRoot = string.IsNullOrWhiteSpace(context.ProjectRoot) ? Directory.GetParent(UnityEngine.Application.dataPath)?.FullName : context.ProjectRoot;
            var reportRoot = UnityQueriesReportWriter.GetReportRootAbsolutePath(context);
            if (string.IsNullOrWhiteSpace(projectRoot) || string.IsNullOrWhiteSpace(reportRoot))
            {
                failure = UnityQueriesResult.InvalidArgs("AGENTBRIDGE_REPORT_ROOT_UNAVAILABLE", "The Agent Bridge report root could not be resolved.");
                return false;
            }
            ...
            {
                failure = UnityQueriesResult.InvalidArgs("AGENTBRIDGE_REPORT_PATH_OUTSIDE_ROOT", "reportPath must resolve within the Agent Bridge report root.");
                return false;
            }
        ...
        {
            return string.Equals(report.Value<string>("schemaVersion"), UnityQueriesJsonUtil.CurrentSchemaVersion, StringComparison.Ordinal) &&
