...
namespace UnityMcp.BuiltInPlugins.UnityQueries
{
    ...
    {
        public static bool TryNormalizeFolders(string[] folders, out string[] normalizedFolders, out UnityMcpToolResult failure)
        {
            ...
            for (var index = 0; index < folders.Length; index++)
            {
                ...
                {
                    failure = UnityQueriesResult.InvalidArgs("AGENTBRIDGE_ASSET_FOLDER_INVALID", "folders must resolve to Unity project paths under Assets/.");
                    return false;
                }

                if (!AssetDatabase.IsValidFolder(rawFolder))
                {
                    failure = UnityQueriesResult.InvalidArgs("AGENTBRIDGE_ASSET_FOLDER_INVALID", $"Folder '{rawFolder}' is not a valid Unity asset folder.");
                    return false;
                }
