...
{
    [AgentTool("unity.get_gameobject_component_info")]
    public sealed class UnityGameObjectComponentInfoTool : IAgentTool
    {
        public ToolDescriptor Descriptor { get; } = new ToolDescriptor
        {
            ...
            ArgsSchemaPath = "Documentation~/schemas/unity.get_gameobject_component_info.args.schema.json"
        };
        ...
        public ToolResult Execute(AgentToolContext context, IAgentCancellation cancellation)
        {
            ...
            {
                return ToolResult.InvalidArgs("AGENTBRIDGE_PROPERTY_MODE_INVALID", "propertyMode must be one of: debug, serialized.");
            }

            if (args.propertyLimit < 0 || args.propertyLimit > AssetQueryContract.MaxPropertyLimit)
            {
                return ToolResult.InvalidArgs("AGENTBRIDGE_PROPERTY_LIMIT_INVALID", $"propertyLimit must be in the range 0..{AssetQueryContract.MaxPropertyLimit}.");
            }

            if (args.arrayElementLimit < 0 || args.arrayElementLimit > AssetQueryContract.MaxArrayElementLimit)
            {
                return ToolResult.InvalidArgs("AGENTBRIDGE_ARRAY_ELEMENT_LIMIT_INVALID", $"arrayElementLimit must be in the range 0..{AssetQueryContract.MaxArrayElementLimit}.");
            }

            if (args.stringMaxLength < AssetQueryContract.MinStringMaxLength || args.stringMaxLength > AssetQueryContract.MaxStringMaxLength)
            {
                return ToolResult.InvalidArgs("AGENTBRIDGE_STRING_MAX_LENGTH_INVALID", $"stringMaxLength must be in the range {AssetQueryContract.MinStringMaxLength}..{AssetQueryContract.MaxStringMaxLength}.");
            }

            if (args.componentIndex < -1)
            {
                return ToolResult.InvalidArgs("AGENTBRIDGE_COMPONENT_INDEX_OUT_OF_RANGE", "componentIndex must be greater than or equal to 0.");
            }
            ...
            {
                return ToolResult.InvalidArgs("AGENTBRIDGE_COMPONENT_INDEX_OUT_OF_RANGE", "componentIndex is outside the target GameObject component list.");
            }
            ...
            var plannedReportPath = AgentBridgeReportWriter.GetReportRelativePath(context.Settings, context.Command.commandId, "get_gameobject_component_info");
            ...
            var result = new ToolResult
            {
                ...
                reportPath = AgentBridgeReportWriter.WriteReport(context.Settings, context.Command.commandId, "get_gameobject_component_info", report)
            };
            return result;
        }
